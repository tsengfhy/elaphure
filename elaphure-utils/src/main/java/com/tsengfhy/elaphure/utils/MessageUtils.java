...
public final class MessageUtils {
    ...
    private static Optional<String> doGetMessage(String code, @Nullable Object[] args, @Nullable String defaultMessage) {
        Optional<MessageSource> optional = Optional.ofNullable(messageSource);
        if (optional.isEmpty()) {
            log.warn("MessageUtils initialization failed");
        }
        return optional.map(messageSource -> messageSource.getMessage(code, args, defaultMessage, LocaleContextHolder.getLocale()));
    }
}
