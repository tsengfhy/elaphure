...
public class ParameterMappingHttpServletRequest extends HttpServletRequestWrapper {

    private final Function<String, String> mapper;
    private byte[] body;

    public ParameterMappingHttpServletRequest(HttpServletRequest request, Function<String, String> mapper) {
        super(request);
    ...
    public ServletInputStream getInputStream() throws IOException {
        if (shouldParse()) {
            if (body == null) {
                body = handleParse(IOUtils.toByteArray(super.getInputStream()));
            }
    ...
    protected byte[] handleParse(byte[] source) {
        return JsonUtils.toJson(mapParameterBody(JsonUtils.fromJson(new String(source), JsonElement.class))).getBytes();
    }

    private JsonElement mapParameterBody(JsonElement json) {
        return Optional.ofNullable(json)
                .filter(element -> !element.isJsonNull())
                .map(element -> {
                    if (element.isJsonArray()) {
                        JsonArray array = new JsonArray();
                        element.getAsJsonArray().forEach(item -> array.add(mapParameterBody(item)));
                        return array;
                    } else if (element.isJsonObject()) {
                        JsonObject object = new JsonObject();
                        element.getAsJsonObject().entrySet().forEach(entry -> object.add(entry.getKey(), mapParameterBody(entry.getValue())));
                        return object;
                    } else {
                        return new JsonPrimitive(mapParameterValue(element.getAsString()));
                    }
                })
